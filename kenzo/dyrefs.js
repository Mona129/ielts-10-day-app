/* Douyin-only immersive reference feed: the same 8 videos used in 04 Content. */
(function(){
  ...
  const DEF=[
    ...
    {slot:'ref-1',name:'Paper Cutout OOTD',tw:'Paper Cutout OOTD',src:'https://mysbpummazvgyesuhlfg.supabase.co/storage/v1/object/public/kenzo-social-media/slots/ref-1-1788279938913.mov',desc:'纸片 / 拼贴式穿搭。',descTw:'紙片 / 拼貼式穿搭。'},
    {slot:'ref-2',name:'Product Scale Shift',tw:'Product Scale Shift',src:'https://mysbpummazvgyesuhlfg.supabase.co/storage/v1/object/public/kenzo-social-media/slots/ref-3-1788279989515.mov',desc:'比例变化制造第一眼停留。',descTw:'比例變化製造第一眼停留。'},
    {slot:'ref-3',name:'Product Detail / Label',tw:'Product Detail / Label',src:'https://mysbpummazvgyesuhlfg.supabase.co/storage/v1/object/public/kenzo-social-media/slots/ref-2-1788279978290.mov',desc:'从标签 / 产品细节开始。',descTw:'從標籤 / 產品細節開始。'},
    {slot:'ref-4',name:'Product Grid Game',tw:'Product Grid Game',src:'https://mysbpummazvgyesuhlfg.supabase.co/storage/v1/object/public/kenzo-social-media/slots/ref-4-1788280004014.mov',desc:'格子 / 选择 / 抽卡机制。',descTw:'格子 / 選擇 / 抽卡機制。'},
    {slot:'ref-5',name:'Outfit Receipt',tw:'Outfit Receipt',src:'https://mysbpummazvgyesuhlfg.supabase.co/storage/v1/object/public/kenzo-social-media/slots/ref-5-1788280011260.mov',desc:'扫描、识别、打印一套 Look。',descTw:'掃描、辨識、列印一套 Look。'},
    {slot:'ref-6',name:'Word Drag / 拖拽机制',tw:'Word Drag / 拖曳機制',src:'https://mysbpummazvgyesuhlfg.supabase.co/storage/v1/object/public/kenzo-social-media/slots/ref-6-1788280023661.mov',desc:'拖动动作触发下一段变化。',descTw:'拖曳動作觸發下一段變化。'},
    {slot:'ref-7',name:'Extra Motion / 新增动态',tw:'Extra Motion / 新增動態',src:'https://mysbpummazvgyesuhlfg.supabase.co/storage/v1/object/public/kenzo-social-media/slots/ref-7-1788280034038.mov',desc:'补充动态参考，继续扩玩法库。',descTw:'補充動態參考，繼續擴充玩法庫。'}
  ];
  ...
  async function readCloud(){
    try{const r=await fetch(SUPA+'/rest/v1/kenzo_app_state?id=eq.main&select=content',{headers:H,cache:'no-store'});if(!r.ok)return;const a=await r.json();cloud=a[0]?.content||{media:{}}}catch(e){console.error('dyr cloud',e)}
  }
  function srcFor(i){return cloud.media?.[DEF[i].slot]?.url||DEF[i].src}
  function render(dir=0){
    ensure();const d=DEF[idx],v=overlay.querySelector('.dyr-video');locked=true;v.classList.remove('outUp','outDown');if(dir)v.classList.add(dir>0?'outUp':'outDown');
  ...
  function next(){idx=idx>=DEF.length-1?0:idx+1;render(1)}
  function prev(){idx=idx<=0?DEF.length-1:idx-1;render(-1)}

  function inject(){
    let isDy=false;try{isDy=S.world==='dy'}catch(e){}if(!isDy)return;
    const strip=document.querySelector('.dyStrip');if(strip&&!strip.querySelector('[data-a="dyRefs"]')){const b=document.createElement('button');b.className='pill dyr-launch';b.dataset.a='dyRefs';b.textContent=t('参考案例','參考案例');const pills=[...strip.querySelectorAll('.pill')];const after=pills.find(x=>/内容|內容/.test(x.textContent||''));after?after.after(b):strip.appendChild(b)}
    const guide=document.querySelector('#guideList');if(guide&&!guide.querySelector('[data-a="dyRefs"]')){const b=document.createElement('button');b.className='gitem';b.dataset.a='dyRefs';b.innerHTML=`<small>VIDEO</small><b>${t('参考案例','參考案例')}</b>`;guide.appendChild(b)}
  ...
  window.addEventListener('kenzoRefsUpdated',()=>readCloud().then(()=>{if(overlay?.classList.contains('show'))render();inject()}));
