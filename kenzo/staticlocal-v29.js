...
(function(){
  ...
  function ensureFeed(){if(feed)return feed;feed=document.createElement('div');feed.className='v29-feed';feed.innerHTML=`<div class="v29-shell"><video class="v29-video" autoplay loop playsinline controls preload="auto"></video><div class="v29-top"><button class="v29-close">× ${tr('返回','返回')}</button><div class="v29-count"></div></div><div class="v29-bottom"><b></b><p></p><small>${tr('↑ 上滑下一条 · ↓ 下滑上一条','↑ 上滑下一支 · ↓ 下滑上一支')}</small></div></div>`;document.body.appendChild(feed);feed.querySelector('.v29-close').onclick=closeFeed;feed.addEventListener('touchstart',e=>sy=e.changedTouches[0].clientY,{passive:true});feed.addEventListener('touchend',e=>{const d=e.changedTouches[0].clientY-sy;if(Math.abs(d)<45)return;if(d<0&&idx<7){idx++;paint()}else if(d>0&&idx>0){idx--;paint()}},{passive:true});return feed}
  function paint(){const f=ensureFeed(),m=META[idx],v=f.querySelector('video');v.pause();v.src=V[idx];v.defaultMuted=false;v.muted=false;v.volume=1;v.load();v.play().catch(()=>{});f.querySelector('.v29-count').textContent=`${idx+1} / 8`;f.querySelector('.v29-bottom b').textContent=tw()?m[1]:m[0];f.querySelector('.v29-bottom p').textContent=tw()?m[3]:m[2]}
